...
class ImbalancedDatasetSampler(torch.utils.data.sampler.Sampler):
    ...
    def __init__(self, dataset, indices=None, num_samples=None):
        ...
        per_cls_weights = [float(label_to_count[0]) / label_to_count[i] for i in range(len(label_to_count))]
        #per_cls_weights = [math.sqrt(float(label_to_count[0]) / label_to_count[i]) for i in range(len(label_to_count))]
...
class SmoothImbalancedDatasetSampler(torch.utils.data.sampler.Sampler):
...
class ClassBalancedImbalancedDatasetSampler(torch.utils.data.sampler.Sampler):
    ...
    def __init__(self, dataset, indices=None, num_samples=None):
        ...
        beta = 0.9999
        effective_num = 1.0 - np.power(beta, label_to_count)
        per_cls_weights = [1.0] * 5
        min_weights = np.max(effective_num) / np.min(effective_num)
        per_cls_weights.extend([min_weights] * 5)
...
class ClassBalancedImbalancedDatasetSamplerExp(torch.utils.data.sampler.Sampler):
...
def evaluate1(model, test_loader, device = 'cuda'):
